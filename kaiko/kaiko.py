...
class KaikoClient:
    ...
    def find_pools_containing_symbol(self, symbol: str) -> pd.DataFrame:
        """
        Returns all the pools containing a certain token
        parameter: token symbol 
        """
        if self.all_pools is None:
            self.load_catalogs()
        return self.all_pools[self.all_pools['tokens'].apply(lambda x: False if x is None else any([False if y is None else y['symbol'].lower() == symbol.lower() for y in x]))]

    def find_pools_containing_address(self, address: str) -> pd.DataFrame:
        """
        Returns all the pools containing a certain token
        ...
        if self.all_pools is None:
            self.load_catalogs()
        return self.all_pools[self.all_pools['tokens'].apply(lambda x: False if x is None else any([False if y is None else y['address'].lower() == address.lower() for y in x]))]
...
class KaikoData:
    ...
    def __init__(self, endpoint: str, req_params: dict, params: dict = {}, client: KaikoClient = None, pagination: bool = False, extra_args: dict = {}, **kwargs):
        assert not(client is None), "Need to give a client that is not None"
        self.client = client 
    ...
    def request_next(self, n_next: int = 1, keep_previous_data: bool = False) -> pd.DataFrame:
        if not(self.next_url is None):
            df, self.query_api, query_res = ut.request_next_df(self.next_url, 
                                                                    return_query = True, 
                                                                    return_res = True,
                                                                    headers = self.client.headers, 
                                                                    df_formatter = self.df_formatter,
            ...
            if keep_previous_data:
                ...
                self.query_res['data'] += query_res['data']
...
class OrderBookSnapshots(KaikoData):
    ...
    def __init__(self, exchange: str, instrument: str, type_of_ob: cste.TYPE_OF_OB_SNAPSHOTS = 'Full', instrument_class: str = 'spot', params: dict = dict(page_size = 100),
...
class OrderBookAggregations(KaikoData):
    ...
    bid_slippage	The average percentage of price slippage for a market sell order over a specified interval.
    """
    def __init__(self, exchange: str, instrument: str, type_of_ob: cste.TYPE_OF_OB_AGGREGATIONS = 'Full', instrument_class: str = 'spot', params: dict = dict(page_size=100),
                data_version: str = 'latest', client: KaikoClient = None, **kwargs):
...
class Aggregates(KaikoData):
    ...
    """
    def __init__(self, exchange: str, instrument: str, type_of_aggregate: cste.TYPE_OF_AGGREGATES = 'COHLCV_VWAP', instrument_class: str = 'spot', params: dict = dict(page_size=100000), 
                data_version: str = 'latest', client: KaikoClient = None, **kwargs):
...
### add assert V2 for data_version
    ...
    def __init__(self, base_asset: str, quote_asset: str, type_of_pricing: cste.TYPE_OF_PRICING = 'SpotDirectExchangeRate', params: dict = dict(page_size=1000), data_version: str = 'latest', client: KaikoClient = None, **kwargs):
...
### This needs to be changed, the dataframe format is not working for this, amounts are not correct with np.float64
class DEXLiquidity(KaikoData):
    """
    ---------------------------------------------------------------------   Events   ---------------------------------------------------------------------
    ...
    def __init__(self,  type_of_dex_liquidity: cste.TYPE_OF_DEX_LIQUIDITY = "Events", params: dict = dict(), client: KaikoClient = None, **kwargs):
        '''
        Parameters for events are: exchange, pool, pool_contains, block, start_time, end_time, sort, type
        Parameters for snapshots are: pool_address, start_block, end_block, start_time, end_time, sort
        '''
        # Initialize endpoint required parameters
        self.req_params = dict()

        self.parameter_space = 'pool_address,exchange,start_block,end_block,start_time,end_time,sort,page_size'.split(',') if type_of_dex_liquidity == "Snapshots" \
                                else 'exchange,pool_address,block_number,type,start_time,end_time,sort,pool_contains,page_size,start_block,end_block'.split(',')
        endpoint = cste._URL_DEX_LIQUIDITY_SNAPSHOTS if type_of_dex_liquidity == "Snapshots" else cste._URL_DEX_LIQUIDITY_EVENTS
...
### TO BE TESTED NEED TO ASK FOR ACCESS
...
class FX(KaikoData):
    ...
    Parameter	            Required	Description
    base	                Yes	        The base fiat currency.
    continuation_token	    No	        See Pagination.
    data_version	        Yes	        The data version. (v1, v2 ... or latest)
    end_time	            No	        Ending time in ISO 8601 (exclusive).
    interval	            No	        Interval period. Default 1h.
    page_size	            No	        See Pagination (min: 1, default: 100, max: 1000).
    quote	                Yes	        The quote fiat currency.
    start_time	            No	        Starting time in ISO 8601 (inclusive).
    sort	                No	        Return the data in ascending (asc) or descending (desc) order. Default desc
    ...
    """
    def __init__(self, base: str, quote: str, data_version: str = 'latest', params: dict = dict(page_size=1000), client: KaikoClient = None, **kwargs):
        self.req_params = dict(base = base,
                                quote = quote,
                                data_version = data_version)
        self.parameter_space = 'continuation_toke,end_time,interval,page_size,start_time,sort'.split(',')
        endpoint = cste._URL_PRICING_FX
        KaikoData.__init__(self, endpoint, self.req_params, params)
        self._request_api()
